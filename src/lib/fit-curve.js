// This file has been modified from the original to support Javascript ES4 standard used on Toon Boom Harmony application (Yu Ueda).
 ...
 */
function fitCurveMaster() {
	this.fitCurve = function(points, maxError, progressCallback) {
	...
	function fitCubic(points, leftTangent, rightTangent, error, progressCallback) {
		...
		if (points.length === 2) {
			dist = MT.vectorLen(MT.subtract(points[0], points[1])) / 3.0;
			bezCurve = [
				points[0],
				MT.addArrays(points[0], MT.mulItems(leftTangent,  dist)),
				MT.addArrays(points[1], MT.mulItems(rightTangent, dist)),
				points[1]
			];
			return [bezCurve];
		}
		...
		var newData = generateAndReport(points, u, u, leftTangent, rightTangent, progressCallback);
		bezCurve = newData[0];
		maxError = newData[1];
		splitPoint = newData[2];
		...
		if (maxError < (error*error)) {
			...
			for (i = 0; i < MaxIterations; i++) {
				...
				var newData2 = generateAndReport(points, u, uPrime, leftTangent, rightTangent, progressCallback);
				bezCurve = newData2[0];
				maxError = newData2[1];
				splitPoint = newData2[2];
		...
		centerVector = MT.subtract(points[splitPoint-1], points[splitPoint+1]);
		...
		if(centerVector.every(function(val){ return val === 0; })) {
			...
			centerVector = MT.subtract(points[splitPoint-1], points[splitPoint]);
			centerVector[0] = -centerVector[1];	
			centerVector[1] = centerVector[0];
		}
		toCenterTangent = MT.normalize(centerVector);
		//To and from need to point in opposite directions:
		fromCenterTangent = MT.mulItems(toCenterTangent, -1);
	...
	function generateAndReport(points, paramsOrig, paramsPrime, leftTangent, rightTangent, progressCallback) {
		var bezCurve = generateBezier(points, paramsPrime, leftTangent, rightTangent, progressCallback);
		...
		var report = computeMaxError(points, bezCurve, paramsOrig);
		var maxError = report[0];
		var splitPoint = report[1];
	...
	function generateBezier(points, parameters, leftTangent, rightTangent) {
		...
		//Compute the A's
		A = MT.zeros_Xx2x2(parameters.length);
		for (i = 0, len = parameters.length; i < len; i++) {
			u = parameters[i];
			ux = 1 - u;
			a = A[i];

			a[0] = MT.mulItems(leftTangent,  3 * u  * (ux*ux));
			a[1] = MT.mulItems(rightTangent, 3 * ux * (u*u));
		}
		...
		for (i = 0, len = points.length; i < len; i++) {
			...
			C[0][0] += MT.dot(a[0], a[0]);
			C[0][1] += MT.dot(a[0], a[1]);
			C[1][0] += MT.dot(a[0], a[1]);
			C[1][1] += MT.dot(a[1], a[1]);

			tmp = MT.subtract(points[i], BZ.q([firstPoint, firstPoint, lastPoint, lastPoint], u));
		...
		segLength = MT.vectorLen(MT.subtract(firstPoint, lastPoint));
		epsilon = 1.0e-6 * segLength;
		if (alpha_l < epsilon || alpha_r < epsilon) {
			//Fall back on standard (probably inaccurate) formula, and subdivide further if needed.
			bezCurve[1] = MT.addArrays(firstPoint, MT.mulItems(leftTangent,  segLength / 3.0));
			bezCurve[2] = MT.addArrays(lastPoint,  MT.mulItems(rightTangent, segLength / 3.0));
		} else {
			//First and last control points of the Bezier curve are
			//positioned exactly at the first and last data points
			//Control points 1 and 2 are positioned an alpha distance out
			//on the tangent vectors, left and right, respectively
			bezCurve[1] = MT.addArrays(firstPoint, MT.mulItems(leftTangent,  alpha_l));
			bezCurve[2] = MT.addArrays(lastPoint,  MT.mulItems(rightTangent, alpha_r));
		}
	...
	function reparameterize(bez, points, parameters) {
		...
		return results;
		//*/
		return parameters.map(function(p, i){ return newtonRaphsonRootFind(bez, points[i], p); });
	};
	...
	function newtonRaphsonRootFind(bez, point, u) {
		...
		*/
		...
		var MT = new maths;
		var BZ = new bezier;		
		var d = MT.subtract(BZ.q(bez, u), point),
			qprime = BZ.qprime(bez, u),
			numerator = MT.mulMatrix(d, qprime),
			denominator = MT.sum(MT.squareItems(qprime)) + 2 * MT.mulMatrix(d, BZ.qprimeprime(bez, u));
	...
	function chordLengthParameterize(points) {
		...
		points.forEach(function(p, i){
			currU = i ? prevU + MT.vectorLen(MT.subtract(p, prevP))
					  : 0;
			u.push(currU);
	...
	function computeMaxError(points, bez, parameters) {
		...
		for (i = 0, count = points.length; i < count; i++) {
			...
			v = MT.subtract(BZ.q(bez, t), point);
	...
	var mapTtoRelativeDistances = function (bez, B_parts) {
		...
		for (var i=1; i<=B_parts; i++) {
		  B_t_curr = BZ.q(bez, i/B_parts);

		  sumLen += MT.vectorLen(MT.subtract(B_t_curr, B_t_prev));
	...
	function createTangent(pointA, pointB) {
		var MT = new maths;
		return MT.normalize(MT.subtract(pointA, pointB));
	}
	...
	*/
	function maths(){
		...
		this.mulItems = function(items, multiplier) {
		...
		this.addArrays = function(arr1, arr2) {
			return arr1.map(function(x1, i){ return x1 + arr2[i];});
		};
		this.addItems = function(items, addition) {
		...
		this.vectorLen = function(v) {			
			var y = 0, i = v.length, containsInfinity = false;
			while (i--) {
				var arg = v[i];
	...
	function bezier(){
		...
		this.q = function(ctrlPoly, t) {
			var tx = 1.0 - t;
			var pA = MT.mulItems( ctrlPoly[0],      tx * tx * tx ),
				pB = MT.mulItems( ctrlPoly[1],  3 * tx * tx *  t ),
				pC = MT.mulItems( ctrlPoly[2],  3 * tx *  t *  t ),
				pD = MT.mulItems( ctrlPoly[3],       t *  t *  t );
			return MT.addArrays(MT.addArrays(pA, pB), MT.addArrays(pC, pD));
		};

		//Evaluates cubic bezier first derivative at t, return point
		this.qprime = function(ctrlPoly, t) {
			var tx = 1.0 - t;
			var pA = MT.mulItems( MT.subtract(ctrlPoly[1], ctrlPoly[0]),  3 * tx * tx ),
				pB = MT.mulItems( MT.subtract(ctrlPoly[2], ctrlPoly[1]),  6 * tx *  t ),
				pC = MT.mulItems( MT.subtract(ctrlPoly[3], ctrlPoly[2]),  3 *  t *  t );
			return MT.addArrays(MT.addArrays(pA, pB), pC);
		};

		//Evaluates cubic bezier second derivative at t, return point
		this.qprimeprime = function(ctrlPoly, t) {
			return MT.addArrays(MT.mulItems( MT.addArrays(MT.subtract(ctrlPoly[2], MT.mulItems(ctrlPoly[1], 2)), ctrlPoly[0]),  6 * (1.0 - t) ),
								   MT.mulItems( MT.addArrays(MT.subtract(ctrlPoly[3], MT.mulItems(ctrlPoly[2], 2)), ctrlPoly[1]),  6 *        t  ));
		};
	}
}
exports.fitCurveMaster = fitCurveMaster;